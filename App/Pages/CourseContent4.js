...
export default function CourseContent4({ route, navigation }) {
    const { topic = "Default Topic" } = route.params || {}; // Sử dụng giá trị mặc định nếu không có topic
    const colors = ['#1D92FF', '#1D92FF', '#1D92FF', '#1D92FF', '#D6DEE5']; // Thay đổi màu của dấu gạch chân thứ 2
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity
                ...
                onPress={() => { navigation.navigate('CourseContentFinish') }}
            >
